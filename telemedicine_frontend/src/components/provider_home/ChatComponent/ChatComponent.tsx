...
import { getListOfPatients, getMessageProviderInitial, sendMessage } from '@/redux/slice/messageSlice';
...
const ChatComponent: React.FC = () => {
  ...
  const usersData :User[]=useAppSelector((state)=>state.message.listOfPatients);
  const admin={
    userID: 252,
    firstName: "Ishaan",
    lastName: "Gangrade",
  ...
  const initialMessages:Message[]=useAppSelector((state)=>state.message.initialMessages)

  const user : User=useAppSelector((state)=>state.providerProfile.user);

  useEffect(()=>{
    const id=localStorage.getItem('id') 
    if(id){
      dispatch(getListOfPatients({id:+id}))
      setMyId(+id);
      dispatch(getMessageProviderInitial({id:+id}));
    ...
    >
      ...
      <Box flex={2} display="flex" flexDirection="column" sx={{ width: "920px", border: "1px solid black", backgroundColor: 'rgba(255, 255, 255, 0.8)' }}>
        <Box flex={1} overflow="auto" paddingBottom={0} display="flex" flexDirection="column" sx={{ height: "50px" }}>
          ...
          sx={{backgroundImage: 'url(/assets/chat-bg.jpg)', 
            backgroundSize: 'cover',
            backgroundPosition: 'center',}}
          >
