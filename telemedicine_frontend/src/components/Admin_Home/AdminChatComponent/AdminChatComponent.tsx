...
import { getListOfPatients, getListOfProviders, getListOfUsers, getMessageAdminInitial, getMessagePatientInitial, getMessageProviderInitial, sendMessage } from '@/redux/slice/messageSlice';
...
import { getAdminUser } from '@/redux/slice/usersSlice';
...
const AdminChatComponent: React.FC = () => {
  ...
  const usersData :User[]=useAppSelector((state)=>state.message.listOfUsers);
  const initialMessages:Message[]=useAppSelector((state)=>state.message.initialMessagesAdmin)
  const user={
    userID: 252,
    firstName: "Ishaan",
    lastName: "Gangrade",
  ...
  useEffect(()=>{
    ...
    if(id){
      dispatch(getListOfUsers({id:+id}))
      setMyId(+id);
      dispatch(getMessageAdminInitial({id:+id}));
