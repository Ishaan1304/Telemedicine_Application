...
import styles from './ProviderUserManagement.module.css';
...
const ProviderUserManagement: React.FC = () => {
  const dispatch = useAppDispatch();
  const allUsers = useAppSelector(
    (state: { users: InitialStateUsers }) => state.users.users
  );

  const [providerList, setProviderList] = useState<User[]>([]);
  ...
  useEffect(() => {
    setProviderList(allUsers.filter(user => user.role === 'PROVIDER'));
  }, [allUsers]);
  ...
  const handleDelete = (userID: number) => {
    ...
    setProviderList(providerList.filter(user => user.userID !== userID));
    setShowDeleteModal(false);
  };

  const filteredUsers = providerList.filter(user =>
...
export default ProviderUserManagement;
